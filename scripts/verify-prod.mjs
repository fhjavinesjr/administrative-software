...
const artifactDir = path.join(root, "dist", "administrative");
...
if (packagedEnvironment.PORT !== "3082") {
  throw new Error("The packaged frontend port is incorrect.");
}

const server = spawn(process.execPath, ["server.js"], {
  cwd: artifactDir,
  env: { ...process.env, HOSTNAME: "127.0.0.1", PORT: "3082" },
...
try {
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    try {
      pageResponse = await fetch("http://127.0.0.1:3082/administrative/login");
      break;
    } catch {
      await delay(500);
    }
  ...
  if (!pageResponse?.ok) {
    throw new Error(`Packaged server did not return HTTP 200 for /administrative/login.\n${output}`);
  }
  ...
  const runtimeResponse = await fetch("http://127.0.0.1:3082/runtime-config.js");
  ...
  console.log("administrative production artifact smoke test passed:");
  console.log("- node server.js listened on port 3082");
  console.log("- GET /administrative/login returned HTTP 200");
