...
export default function DayEquivalentTable() {
  const canAdd = localStorageUtil.canAdd("admin.dayEquivalent");
  const canEdit = localStorageUtil.canEdit("admin.dayEquivalent");
  const canDelete = localStorageUtil.canDelete("admin.dayEquivalent");
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await fetchWithAuth(
        `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentHours/get-all`,
      );
  ...
  const fetchDayEquivalent = useCallback(
  ...
  const fetchMinutesHistory = useCallback(async () => {
    try {
      const res = await fetchWithAuth(
        `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentMinutes/get-all`,
      );
      if (!res.ok)
        throw new Error("Failed to fetch day equivalent minutes history");
  ...
  const fetchDayEquivalentMinutes = useCallback(
    async (dateToFetch?: string) => {
      const dateValue = dateToFetch || effectivityDate;
      if (!dateValue) return;
      ...
      try {
        ...
        Swal.fire(
          "Error",
          "Failed to load Day Equivalent Minutes table",
          "error",
        );
      } finally {
        setLoading(false);
  ...
  const addRow = () => {
    ...
    setRows((prev) => [...prev, { hours: "", hoursEquivalent: "" }]);
  };
  const removeRow = () => {
    setRows((prev) => {
      if (prev.length <= 1) return prev;
      const removed = prev[prev.length - 1];
      if (removed?.dayEquivalentHoursId && !canDelete) {
        void Swal.fire({
          icon: "warning",
  ...
  const addMinuteRow = () => {
    if (!canAdd && !canEdit) return;
    setMinuteRows((prev) => [...prev, { minutes: "", minutesEquivalent: "" }]);
  };
  const removeMinuteRow = () => {
    setMinuteRows((prev) => {
      if (prev.length <= 1) return prev;
      const removed = prev[prev.length - 1];
      if (removed?.dayEquivalentMinutesId && !canDelete) {
        void Swal.fire({
          icon: "warning",
          title: "Permission denied",
          text: "Deleting a saved row requires Delete permission.",
  ...
  const updateMinuteRow = (
    index: number,
    field: keyof MinuteItem,
    value: string,
  ) => {
    setMinuteRows((prev) =>
      prev.map((r, i) => (i === index ? { ...r, [field]: value } : r)),
  ...
  const checkExistingByEffectivityDate = async (date: string) => {
    const res = await fetchWithAuth(
      `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentHours/getBy/${toCustomFormat(date, false)}`,
    );
  ...
  const checkExistingMinutesByEffectivityDate = async (date: string) => {
    const res = await fetchWithAuth(
      `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentMinutes/getBy/${toCustomFormat(date, false)}`,
    );
  ...
  const handleSave = async () => {
    const editingExisting = rows.some((r) => r.dayEquivalentHoursId);
    ...
    try {
      ...
      if (rows.some((r) => !r.hours.trim() || !r.hoursEquivalent.trim())) {
        Swal.fire(
          "Validation",
          "Hours and Equivalent cannot be empty",
          "warning",
        );
        return;
      }
      ...
      const url = isEditing
        ? `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentHours/update`
        : `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentHours/create`;
      ...
      if (deletedRows.length > 0) {
        try {
          const delPayload = deletedRows.map((r) => ({
            dayEquivalentHoursId: r.dayEquivalentHoursId,
          }));
          await fetchWithAuth(
            `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentHours/deleteById`,
            {
              method: "DELETE",
  ...
  const handleEditHistory = async (selectedDate: string) => {
    ...
    try {
      setLoading(true);
      const res = await fetchWithAuth(
        `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentHours/getBy/${toCustomFormat(selectedDate, false)}`,
      );
      if (!res.ok)
        throw new Error("Failed to fetch day equivalent for the selected date");
      const data: HourItem[] = await res.json();
      if (data.length > 0) {
        setRows(
          data.map((item) => ({
  ...
  const handleEditMinutesHistory = async (selectedDate: string) => {
    ...
    try {
      setLoading(true);
      const res = await fetchWithAuth(
        `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentMinutes/getBy/${toCustomFormat(selectedDate, false)}`,
      );
      if (!res.ok)
        throw new Error(
          "Failed to fetch day equivalent minutes for the selected date",
        );
      const data: MinuteItem[] = await res.json();
      if (data.length > 0) {
        setMinuteRows(
      ...
      Swal.fire(
        "Error",
        "Failed to load selected Day Equivalent Minutes",
        "error",
      );
    } finally {
      setLoading(false);
  ...
  const handleDeleteHistory = async (date: string) => {
    if (!canDelete) return;
    const confirm = await Swal.fire({
      text: `Delete day equivalent effective ${date}?`,
      icon: "warning",
      showCancelButton: true,
    ...
    try {
      await fetchWithAuth(
        `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentHours/delete/${toCustomFormat(date, false)}`,
        { method: "DELETE" },
      );
  ...
  const handleDeleteMinutesHistory = async (date: string) => {
    if (!canDelete) return;
    const confirm = await Swal.fire({
      text: `Delete day equivalent minutes effective ${date}?`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonText: "Delete",
    });
    ...
    try {
      await fetchWithAuth(
        `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentMinutes/delete/${toCustomFormat(date, false)}`,
        { method: "DELETE" },
      );
      Swal.fire("Deleted", "History deleted successfully", "success");
      fetchMinutesHistory();
  ...
  const handleSaveMinutes = async () => {
    const editingExisting = minuteRows.some((r) => r.dayEquivalentMinutesId);
    if (editingExisting ? !canEdit : !canAdd) {
      void Swal.fire({
        icon: "warning",
        title: "Permission denied",
    ...
    if (deletedMinuteRows.length > 0 && !canDelete) {
      void Swal.fire({
        icon: "warning",
        title: "Permission denied",
        text: "Deleting saved rows requires Delete permission.",
    ...
    try {
      ...
      if (
        minuteRows.some((r) => !r.minutes.trim() || !r.minutesEquivalent.trim())
      ) {
        Swal.fire(
          "Validation",
          "Minutes and Equivalent cannot be empty",
          "warning",
        );
        return;
      }
      ...
      setLoading(true);
      const existingData =
        await checkExistingMinutesByEffectivityDate(effectivityDate);
      ...
      const payload = minuteRows.map((row) => ({
        ...row,
        effectivityDate: toCustomFormat(effectivityDate, false),
      }));
      const url = isEditing
        ? `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentMinutes/update`
        : `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentMinutes/create`;
      ...
      if (deletedMinuteRows.length > 0) {
        try {
          const delPayload = deletedMinuteRows.map((r) => ({
            dayEquivalentMinutesId: r.dayEquivalentMinutesId,
          }));
          await fetchWithAuth(
            `${API_BASE_URL_ADMINISTRATIVE}/api/dayEquivalentMinutes/deleteById`,
            {
              method: "DELETE",
              body: JSON.stringify(delPayload),
            },
      ...
      Swal.fire(
        "Success",
        "Day Equivalent Minutes saved successfully",
        "success",
      );
        ...
        <div className={modalStyles.modalBody}>
          <div className={DayEquivalentTableStyle.DayEquivalent}>
            <div className={DayEquivalentTableStyle.tabsHeader}>
              <button
                className={`${DayEquivalentTableStyle.tabButton} ${activeTab === "hour" ? DayEquivalentTableStyle.active : ""}`}
                onClick={() => setActiveTab("hour")}
              >
                Hours
              </button>
              <button
                className={`${DayEquivalentTableStyle.tabButton} ${activeTab === "minute" ? DayEquivalentTableStyle.active : ""}`}
            ...
            <div className={DayEquivalentTableStyle.DayEquivalentTable}>
              <div className={DayEquivalentTableStyle.toolbar}>
                <button
                  className={DayEquivalentTableStyle.newButton}
                  onClick={
                    activeTab === "hour" ? handleSave : handleSaveMinutes
                  }
                  disabled={
                    loading ||
                    (activeTab === "hour"
                      ? rows.some((r) => r.dayEquivalentHoursId)
                        ? !canEdit
                        : !canAdd
                      : minuteRows.some((r) => r.dayEquivalentMinutesId)
                        ? !canEdit
                        : !canAdd)
                  }
                >
                ...
                <button
                  className={DayEquivalentTableStyle.clearButton}
                  onClick={
                    activeTab === "hour" ? handleClear : handleClearMinutes
                  }
                >
                  ✖ Clear
                </button>
                  ...
                  <table className={DayEquivalentTableStyle.dayTable}>
                    ...
                    <tbody>
                        ...
                        <tr key={idx}>
                          <td>
                            <input
                              type="text"
                              className={
                                DayEquivalentTableStyle.earnedLeaveInput
                              }
                              value={row.hours}
                              onChange={(e) =>
                      ...
                      <tr>
                        <td
                          colSpan={2}
                          className={DayEquivalentTableStyle.addTableRow}
                        >
                          <button
                            className={DayEquivalentTableStyle.myButton}
                            onClick={addRow}
              ...
              <div>&nbsp;</div>
              <div
                className={DayEquivalentTableStyle.DayEquivalentTableHistory}
              >
                <h3 className={DayEquivalentTableStyle.historyTitle}>
                  {activeTab === "hour"
                    ? "Day Equivalent History"
                    : "Day Equivalent Minutes History"}
                </h3>
                    ...
                    {activeTab === "hour" &&
                      history.map((item) => (
                        <tr key={item.effectivityDate}>
                          ...
                          <td>
                            <button
                              className={`${DayEquivalentTableStyle.iconButton} ${DayEquivalentTableStyle.editIcon}`}
                              onClick={() =>
                            ...
                            <button
                              className={`${DayEquivalentTableStyle.iconButton} ${DayEquivalentTableStyle.deleteIcon}`}
                              onClick={() =>
                        ...
                        <tr key={item.effectivityDate}>
                          ...
                          <td>
                            <button
                              className={`${DayEquivalentTableStyle.iconButton} ${DayEquivalentTableStyle.editIcon}`}
                              onClick={() =>
                                handleEditMinutesHistory(item.effectivityDate)
                              }
                              title="Edit"
                              disabled={!canEdit}
                            >
                            ...
                            <button
                              className={`${DayEquivalentTableStyle.iconButton} ${DayEquivalentTableStyle.deleteIcon}`}
                              onClick={() =>
                                handleDeleteMinutesHistory(item.effectivityDate)
                              }
                              title="Delete"
                              disabled={!canDelete}
                            >
