...
export default function ApprovalWorkFlow() {
  const canAdd = localStorageUtil.canAdd("admin.approvalWorkflow");
  const canEdit = localStorageUtil.canEdit("admin.approvalWorkflow");
  const canDelete = localStorageUtil.canDelete("admin.approvalWorkflow");
  ...
  const [designatedPersonnel, setDesignatedPersonnel] = useState<
    ManagePersonnelEntry[]
  >([]);
  ...
  useEffect(() => {
    ...
    setSearch(
      currentEmployee
        ? `[${currentEmployee.employeeNo}] ${currentEmployee.fullName}`
        : "",
    );
    ...
    empSearch.trim().length > 0
      ? allWorkflows.filter((w) => {
          ...
          return (
            ...
            (w.employeeNo ?? "").toLowerCase().includes(q)
          );
        })
      : [];

  const selectedUnit = units.find((u) => u.businessUnitsId === selectedUnitId);
  const selectedRequest = employeeRequests.find(
    (r) => r.employeeRequestId === selectedRequestId,
  );
  ...
  const handleAdd = async () => {
    ...
    const alreadyAdded = workflowEntries.some(
    ...
    if (alreadyAdded) {
      Swal.fire({
        icon: "warning",
        title: "Already Added",
        text: `${selectedEmployee.fullName} is already in the workflow.`,
      });
      return;
    }
    ...
    try {
      const res = await fetchWithAuth(
        `${API_BASE_URL}/api/approval-workflow/save`,
        {
          method: "POST",
      ...
      const personnelEntry = designatedPersonnel.find(
      ...
      setWorkflowEntries((prev) => [
        ...prev,
        {
          ...
          status: personnelEntry
            ? resolveStatus(personnelEntry)
            : "Rank and File",
        },
      ]);
      setSearch("");
        ...
        <div className={modalStyles.modalBody}>
          <form className={styles.ApprovalWorkflowForm}>
                ...
                (empResults.length > 0 ? (
                  <div className={styles.empLookupResults}>
                    <p className={styles.empLookupCount}>
                      <strong>{empResults[0].employeeName}</strong> is assigned
                      to <strong>{empResults.length}</strong> approval workflow
                      {empResults.length !== 1 ? "s" : ""}.
                    </p>
                          ...
                          {empResults.map((r, i) => {
                            ...
                            const reqName =
                              employeeRequests.find(
                                (req) =>
                                  req.employeeRequestId === r.employeeRequestId,
                              )?.name ?? "—";
                            return (
                  ...
                  <p className={styles.noPersonnel}>
                    No approval workflows found for &ldquo;{empSearch}&rdquo;.
                  </p>
                ))}
            </div>
            ...
            >
              ...
              {areas.map((a) => (
                <option key={a.areasId} value={a.areasId}>
                  {a.areasName}
                </option>
              ))}
            </select>
              ...
              <div>
                ...
                >
                  ...
                  {employeeRequests.map((r) => (
                    <option
                      key={r.employeeRequestId}
                      value={r.employeeRequestId}
                    >
                      {r.name}
                    </option>
                  ))}
                ...
                >
                  ...
                  <span>
                    ...
                    {designatedPersonnel.length > 0
                      ? ` (${designatedPersonnel.length})`
                      : ""}{" "}
                    Designated Personnel
                  </span>
                </div>
                <div
                  className={`${styles.containerUnit} ${fieldOne ? styles.open : styles.closed}`}
                >
                    ...
                    <div className={styles.ApprovalWorkflowTable}>
                      <table className={styles.table}>
                        ...
                        <tbody>
                          ...
                          {designatedPersonnel.length === 0 && (
                            <tr>
                              <td colSpan={5} className={styles.noPersonnel}>
                                No designated personnel for this unit.
                              </td>
                            </tr>
                          )}
                        </tbody>
                      ...
                      {workflowEntries.length > 0 && (
                        <tbody>
                          {workflowEntries.map((ent, i) => {
                            const personnel = designatedPersonnel.find(
                              (p) => p.employeeId === ent.employeeId,
                            );
                            const status =
                              ent.status ??
                              (personnel
                                ? resolveStatus(personnel)
                                : "Rank and File");
                            return (
