...
export default function SystemSetup() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isEditing ? !empCanEdit : !empCanAdd) {
      void Swal.fire({
        icon: "warning",
        title: "Permission denied",
        text: isEditing
          ? "You do not have permission to edit settings."
          : "You do not have permission to add settings.",
      });
      return;
    }
    ...
    if (!isEditing) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          fetchWithAuth(
            `${API_BASE_URL_ADMINISTRATIVE}/api/settings/update/${settingsId}`,
            {
              method: "PUT",
              body: JSON.stringify(payload),
            },
        ...
        const mapped = data.map(
          (d: BackendSettings) =>
            ({
              ...
              date: d.systemStartDate
                ? toDateInputValue(d.systemStartDate)
                : "",
  ...
  const handleDelete = (indx: number) => {
    if (!empCanDelete) {
      void Swal.fire({
        icon: "warning",
        title: "Permission denied",
        text: "You do not have permission to delete settings.",
      });
      return;
    }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetchWithAuth(
          `${API_BASE_URL_ADMINISTRATIVE}/api/settings/delete/${target.settingsId}`,
          { method: "DELETE" },
        )
          .then((res) => {
  ...
  const handleEdit = (ent: settingsEntry) => {
    if (!empCanEdit) {
      void Swal.fire({
        icon: "warning",
        title: "Permission denied",
        text: "You do not have permission to edit settings.",
      });
      return;
    }
    setIsEditing(true);
        ...
        <div className={modalStyles.modalBody}>
          <form className={styles.SystemSetup} onSubmit={onSubmit}>
            ...
            <div className={styles.buttonGroup}>
              {["Left Header", "Main Menu", "Right Header"].map(
                (label, index) => {
            ...
            <div className={styles.buttonGroup}>
              <button
                ...
                disabled={
                  (!empCanAdd && !isEditing) || (isEditing && !empCanEdit)
                }
              >
