...
function SsoCallback() {
  ...
  useEffect(() => {
    ...
    void bootstrapSso(code, "administrative")
      .then(() => router.replace("/administrative/welcomepage"))
  ...
  return (
    <main style={{ minHeight: "70vh", display: "grid", placeItems: "center", textAlign: "center" }}>
      <div>
        <h1 style={{ fontSize: 22, marginBottom: 8 }}>{error ? "Unable to sign in" : "Opening Administrative"}</h1>
        <p>{error || "Preparing your employee profile and permissions..."}</p>
        {error && <button type="button" onClick={() => router.replace("/administrative/login")}>Return to login</button>}
      </div>
    </main>
  );
}
