...
export default function PhilHealth() {
  ...
  const canDelete = localStorageUtil.canDelete("admin.philhealth");
  ...
  const loadPhilHealthData = async () => {
    ...
    try {
      ...
      Swal.fire(
        "Error",
        "Failed to load PhilHealth contribution data.",
        "error",
      );
    } finally {
      setLoading(false);
          ...
          {philHealthData.length > 0 && (
            <div>
              <h4 className={styles.tableHeader}>
                PHILHEALTH SCHEDULE OF CONTRIBUTIONS FOR EMPLOYED MEMBERS
              </h4>
