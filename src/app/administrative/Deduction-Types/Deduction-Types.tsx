...
const defaultFlags = (): Omit<
  DeductionTypeEntry,
  "deductionTypeId" | "accountingCode" | "name"
> => ({
...
export default function DeductionTypes() {
  const canAdd = localStorageUtil.canAdd("admin.deductionTypes");
  const canEdit = localStorageUtil.canEdit("admin.deductionTypes");
  const canDelete = localStorageUtil.canDelete("admin.deductionTypes");
  ...
  const loadData = useCallback(async () => {
    try {
      const res = await fetchWithAuth(
        `${API_BASE_URL}/api/deductionType/get-all`,
      );
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!isEditing) {
        const res = await fetchWithAuth(
          `${API_BASE_URL}/api/deductionType/create`,
          {
            method: "POST",
        ...
        const res = await fetchWithAuth(
          `${API_BASE_URL}/api/deductionType/update/${editItem.deductionTypeId}`,
          {
            method: "PUT",
            body: JSON.stringify(payload),
          },
  ...
  const handleDelete = (item: DeductionTypeEntry) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed && item.deductionTypeId) {
        try {
          const res = await fetchWithAuth(
            `${API_BASE_URL}/api/deductionType/delete/${item.deductionTypeId}`,
            { method: "DELETE" },
          );
          if (!res.ok) throw new Error();
